...
\smallskip
\noindent\textbf{Fréquence fondamentale et harmoniques :}
\begin{itemize}[leftmargin=1.5em, topsep=2pt, itemsep=1pt]
  \item Le terme $a_0$ (rang $n = 0$) est la \textbf{composante continue} (DC), de fréquence nulle.
  \item Le rang $n = 1$ correspond à la \textbf{fondamentale}, de fréquence $f_1 = f_0 = \dfrac{1}{T}$.
  \item Le rang $n \geq 2$ correspond aux \textbf{harmoniques} : l'harmonique de rang $n$ a pour fréquence : \\ $f_n = n \times f_0$.
\end{itemize}
...
\begin{tcolorbox}[qcmbox={Exercice 1 \, -- \, QCM \hfill \normalfont\textit{(5 points)}}]
  Pour chaque question, \textbf{une seule} réponse est correcte.
\end{tcolorbox}
...
\]
$P$ est la \textbf{puissance moyenne} du signal (en $\mathrm{W}$ si $s$ est en volts).
\end{tcolorbox}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  \item Identifier les coefficients $a_0$, $a_1$, $a_2$, $a_3$ et $a_4$ du développement.
        \hfill \textit{(1 pt)}
  \begin{tcolorbox}[reponsebox, height=2.8cm]\end{tcolorbox}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \addplot[red!70!black, line width=7pt]
        coordinates {(0,0)(0,3)};
      \node[above, font=\small\bfseries, red!70!black] at (axis cs:200,3) {$3$};
  ...
  \item Calculer le \textbf{taux de distorsion harmonique} (TDH) du signal, défini par :
        ...
        \begin{tcolorbox}[colback=yellow!10!white, colframe=yellow!60!black,
          ...
          \small\textbf Si TDH $< 10\,\%$ : signal peu distordu \quad|\quad
          $10\,\% \leq$ TDH $\leq 30\,\%$ : distorsion modérée \quad|\quad \\
          TDH $> 30\,\%$ : signal très distordu.
        \end{tcolorbox}
        Commenter la valeur obtenue. \hfill \textit{(2 pts)}
  \begin{tcolorbox}[reponsebox, height=3.5cm]\end{tcolorbox}
