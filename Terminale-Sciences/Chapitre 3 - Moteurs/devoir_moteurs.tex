...
\begin{minipage}[h]{0.60\linewidth}
	...
	\begin{tikzpicture}[font=\small, x=1cm, y=1cm]
		% Dimensions : 8.4 cm de large, 5 cm de haut
		% 6 colonnes de 1.4 cm chacune
		\def\W{8.4}

		% Bordure extérieure épaisse
		\draw[line width=2pt] (0,0) rectangle (\W,5);

		% Lignes horizontales
		\draw[thick] (0,4.0) -- (\W,4.0); % séparation titre
		\draw[thick] (0,3.2) -- (\W,3.2); % séparation VIDE/ASYNC
		\draw[thick] (0,2.5) -- (\W,2.5); % séparation Mot-Nr/Type
		\draw[thick] (0,1.35) -- (\W,1.35); % séparation labels/valeurs

		% Ligne verticale centrale (rangées supérieures)
		\draw[thick] (4.2,2.5) -- (4.2,5);

		% Lignes verticales section données (6 colonnes de 1.4 cm)
		\foreach \x in {1.4,2.8,4.2,5.6,7.0}{
			\draw[thick] (\x,0) -- (\x,2.5);
		}

		% --- Titre ---
		\node[font=\bfseries\normalsize, align=center] at (2.1,4.5)
			{MOTEUR\\ASCENSEUR};
		\node[font=\footnotesize, align=center] at (6.3,4.5)
			{LOHER \& SÖHNE\\RUHSTORF ROT};

		% --- VIDE / ASYNCHRONE ---
		\node[font=\footnotesize] at (2.1,3.6) {VIDE 0310};
		\node[font=\bfseries\footnotesize, align=center] at (6.3,3.6)
			{ASYNCHRONE / 3$\,\sim$ / $\triangle$};

		% --- Mot-Nr / Type ---
		\node[font=\footnotesize] at (2.1,2.875) {Mot.-Nr.\ 5\,923\,567};
		\node[font=\bfseries\footnotesize] at (6.3,2.875) {Type : ADK-4};

		% --- Étiquettes colonnes ---
		\node[font=\scriptsize, align=center] at (0.7,1.9) {Tension\\(V)};
		\node[font=\scriptsize, align=center] at (2.1,1.9) {Intensité\\(A)};
		\node[font=\scriptsize, align=center] at (3.5,1.9) {$\cos\varphi$};
		\node[font=\scriptsize, align=center] at (4.9,1.9) {$P_{\rm utile}$\\(kW)};
		\node[font=\scriptsize, align=center] at (6.3,1.9) {Puissance\\(CV)};
		\node[font=\scriptsize, align=center] at (7.7,1.9) {Vitesse\\(tr/min)};

		% --- Valeurs ---
		\node[font=\bfseries\normalsize] at (0.7,0.65) {400};
		\node[font=\bfseries\normalsize] at (2.1,0.65) {10,7};
		\node[font=\bfseries\normalsize] at (3.5,0.65) {0,78};
		\node[font=\bfseries\normalsize] at (4.9,0.65) {4,2};
		\node[font=\bfseries\normalsize] at (6.3,0.65) {5,6};
		\node[font=\bfseries\normalsize] at (7.7,0.65) {1350};
	\end{tikzpicture}
